...
def magnus1_mp(H_coeff, t0, tf):
    ...
    int3 = h * H_coeff[2]
    return qt.liouvillian(int1*qt.sigmax() + int2*qt.sigmay() + int3*qt.sigmaz())

def mag2_mp_1(H_coeff, rho0, tlist):
    states = [mp.vec(rho0)]
    
    for i in range(len(tlist) - 1):
        omega = magnus1_mp(H_coeff, tlist[i], tlist[i+1]) + mp.magnus2([H_coeff], qt.Qobj(0), tlist[i], tlist[i+1])
